...
)
{
	...
	// 위 꼭지점과 아래 꼭지점
	VertexPU topVertex;
	topVertex.m_data.m_position = { 0.0f, +RADIUS, 0.0f };
	topVertex.m_data.m_uv = { 0.0f, 0.0f };

	VertexPU bottomVertex;
	bottomVertex.m_data.m_position = { 0.0f, -RADIUS, 0.0f };
	bottomVertex.m_data.m_uv = { 0.0f, 1.0f };
	...
	for (UINT i = 1; i <= STACK - 1; ++i)
	{
		...
		for (UINT j = 0; j <= SLICE; ++j)
		{
			...
			v.m_data.m_position.x = RADIUS * sinf(phi) * cosf(theta);
			v.m_data.m_position.y = RADIUS * cosf(phi);
			v.m_data.m_position.z = RADIUS * sinf(phi) * sinf(theta);

			v.m_data.m_uv.x = theta / DirectX::XM_2PI;
			v.m_data.m_uv.y = phi / DirectX::XM_PI;
