...
void DemoProcess::Initialize(HWND _hwnd)
{
	...
	std::stringstream* temp = m_fms.OpenFile(L"DefaultVS.cso");
	m_renderer->CreateVertexShader("DefaultVS", temp);

	temp = m_fms.OpenFile(L"DefaultPS.cso");
	m_renderer->CreatePixelShader("DefaultPS", temp);
...
{
	std::stringstream* s = m_fms.OpenFile(L"D3DSetting.lua");
	if (nullptr == s)
	{
		return;
	}
...
void DemoProcess::D3DSetting()
{
	...
	std::stringstream* enumSetting = m_fms.OpenFile(L"D3DEnum.lua");
