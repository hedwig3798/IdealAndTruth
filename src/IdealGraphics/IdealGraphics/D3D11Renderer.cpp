...
IE D3D11Renderer::CreateVertexShader(const std::string& _name, std::stringstream* _stream)
{
	...
	if (false == _stream->good())
	{
		return IE::STREAM_ERROR;
	}

	const std::string& vsBuff = _stream->str();

	// 정점 셰이더 객체 생성
	ComPtr<ID3D11VertexShader> vs;
	m_device->CreateVertexShader(vsBuff.data(), vsBuff.size(), nullptr, vs.GetAddressOf());
...
IE D3D11Renderer::CreatePixelShader(const std::string& _name, std::stringstream* _stream)
{
	// 같은 이름의 셰이더가 있다면 무시힌다.
	...
	const std::string& psBuff = _stream->str();

	// 정점 셰이더 객체 생성
	ComPtr<ID3D11PixelShader> ps;
	m_device->CreatePixelShader(psBuff.data(), psBuff.size(), nullptr, ps.GetAddressOf());
